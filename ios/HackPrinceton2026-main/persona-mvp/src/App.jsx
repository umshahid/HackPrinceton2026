...
function AppContent() {
  ...
  const { videoRef, captureFrame, startCapture, cameraReady } = useCamera()
  ...
  const handleStartSession = useCallback(async () => {
    // Screen share must be invoked from a user gesture. Do this first so if
    // the user cancels the picker, we don't start anything else.
    const ok = await startCapture()
    if (!ok) {
      console.warn('Display capture cancelled — not starting session')
      return
    }
    ...
    startSession()
  }, [modelsReady, startSession, startAudioMonitoring, startCapture])
